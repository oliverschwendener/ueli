...
import { EnvironmentVariableSearchPlugin } from "./search-plugins/environment-variable-search-plugin";
...
export class ProductionSearchPluginManager implements SearchPluginManager {
    private readonly plugins: SearchPlugin[];

    public constructor(config: UserConfigOptions, environmentVariableCollection: { [key: string]: string }) {
        this.plugins = [];
        ...
        if (config.features.shortcuts) {
            this.plugins.push(new ShortcutsPlugin(config.shortcuts, config.iconSet.shortcutIcon));
        }
        ...
        if (config.features.environmentVariables) {
            this.plugins.push(new EnvironmentVariableSearchPlugin(environmentVariableCollection, config.iconSet));
        }
