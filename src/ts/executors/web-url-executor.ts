...
export class WebUrlExecutor implements Executor {
    ...
    constructor() {
        this.executor = Injector.getUrlExecutor();
    }

    public execute(url: string): void {
        this.executor.openUrlWithDefaultBrowser(url);
    }

    public isValidForExecution(url: string): boolean {
        let regex = new RegExp(/^((https?:)?[/]{2})?([a-z0-9]+[.])+[a-z]+.*$/i, 'gi');
...
export interface UrlExecutor {
    openUrlWithDefaultBrowser(url: string): void;
}

export class WindowsUrlExecutor implements UrlExecutor {
    public openUrlWithDefaultBrowser(url: string): void {
        childProcess.exec(`start "" "${url}"`, (err, stout, sterr) => {
            if (err) {
                throw err;
            }
        });
...
export class MacOsUrlExecutor implements UrlExecutor {
    public openUrlWithDefaultBrowser(url: string): void {
        childProcess.exec(`open "${url}"`, (err, stout, sterr) => {
            if (err) {
                throw err;
            }
        });
