...
export class MacOsSettingsPlugin implements SearchPlugin {
    ...
    constructor(iconSet: IconSet) {
        ...
        items = items.concat(this.getSystemCommands());
        items = items.concat(this.getSystemSettings());

        this.items = items.map((item: SearchResultItem): SearchResultItem => {
            return {
                description: item.description,
                executionArgument: item.executionArgument,
                icon: iconSet.operatingSystemSettingsIcon,
                name: item.name,
                searchable: [item.name].concat(item.tags),
                tags: item.tags,
            };
        });
    }
    ...
    private getSystemCommands(): SearchResultItem[] {
        ...
        result.forEach((r: SearchResultItem): void => {
            r.description = "macOS";
        });
    ...
    private getSystemSettings() {
        ...
        return result.map((r: SearchResultItem): SearchResultItem => {
            return {
                description: `System Preferences ${UeliHelpers.searchResultDescriptionSeparator} ${basename(r.executionArgument)}`,
                executionArgument: r.executionArgument,
                icon: r.icon,
                name: r.name,
