...
describe(SearchPluginsSearcher.name, (): void => {
    ...
    const appIconStore = new FakeAppIconStore();
    ...
    describe("getSearchResults", (): void => {
        ...
        const searcher = new SearchPluginsSearcher(testConfig, countManager, searchPluginManager, appIconStore);
    ...
    it("should initialize the icon store", (): void => {
        ...
        new SearchPluginsSearcher(testConfig, countManager, searchPluginManager, appIconStore);
        expect(appIconStore.hasBeenInitialized).toBe(true);
    });
});
