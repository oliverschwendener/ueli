...
describe(TokenParser, () => {
    describe(TokenParser.prototype.parse, () => {
        ...
        it("simple literal patterns", () => {
            expect(new TokenParser("Hello").parse()).toEqual(<TokenType[]>[
        ...
        it("simple column patterns", () => {
            expect(new TokenParser("$0").parse()).toEqual(<TokenType[]>[
                { type: "column", index: 0 },
            ]);
            expect(new TokenParser("$1").parse()).toEqual(<TokenType[]>[
                { type: "column", index: 1 },
            ]);
            expect(new TokenParser("$1886").parse()).toEqual(<TokenType[]>[
                { type: "column", index: 1886 },
            ]);
        ...
        it("simple function patterns", () => {
            ...
            expect(new TokenParser("$FUNCTION(test,bla)").parse()).toEqual(<TokenType[]>[
                { type: "function", name: "FUNCTION", params: [[{ type: "literal", value: "test" }], [{ type: "literal", value: "bla" }]] },
            ]);
        });
        ...
        it("nested function patterns", () => {
            ...
            expect(new TokenParser("$OUTER($INNER($1886))").parse()).toEqual(<TokenType[]>[
                { type: "function", name: "OUTER", params: [[{ type: "function", name: "INNER", params: [[{ type: "column", index: 1886 }]] }]] },
            ]);
            expect(new TokenParser("$OUTER($INNER(test))").parse()).toEqual(<TokenType[]>[
                { type: "function", name: "OUTER", params: [[{ type: "function", name: "INNER", params: [[{ type: "literal", value: "test" }]] }]] },
            ]);
            expect(new TokenParser("$OUTER($INNER(test),hallo)").parse()).toEqual(<TokenType[]>[
                { type: "function", name: "OUTER", params: [[{ type: "function", name: "INNER", params: [[{ type: "literal", value: "test" }]] }], [{ type: "literal", value: "hallo" }]] },
            ]);
            expect(new TokenParser("$OUTER($INNER(test),$OTHER(bla))").parse()).toEqual(<TokenType[]>[
                { type: "function", name: "OUTER", params: [[{ type: "function", name: "INNER", params: [[{ type: "literal", value: "test" }]] }], [{ type: "function", name: "OTHER", params: [[{ type: "literal", value: "bla" }]] }]] },
            ]);
        });
