...
describe(TokenEvaluator, () => {
    describe(TokenEvaluator.prototype.evaluate, () => {
        it("evaluate literals", () => {
            ...
            expect(new TokenEvaluator().evaluate([{ type: "literal", value: "$ILookLikeAFunc(,)"}], [])).toEqual("$ILookLikeAFunc(,)");
        });
        ...
        it("evaluate unknown function throws error", () => {
            expect(() => new TokenEvaluator().evaluate([{ type: "function", name: "UNKNOWNFUNC", params: [] }], [])).toThrow("Unknown function: UNKNOWNFUNC");
        });

        it("evaluate complex expression", () => {
            expect(new TokenEvaluator().evaluate([
            ...
            expect(new TokenEvaluator().evaluate([
                ...
                { type: "column", index: 0 },
                { type: "literal", value: ", '" },
                { type: "function", name: "GETDATE", params: [[{ type: "literal", value: "yyyy-MM-dd HH:mm:ss" }]] },
                { type: "literal", value: "', '" },
                { type: "column", index: 1 },
                { type: "literal", value: "');" },
            ], ["123", "Test"])).toMatch(/^INSERT INTO TABLE\(ID, CREATIONDATE, NAME\) VALUES\(123, '\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}', 'Test'\);$/);
        });
    });
});