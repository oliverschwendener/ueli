...
export class UwpAppsRetriever {
    public static getAll(alreadyKnownApps: UwpApplication[]): Promise<UwpApplication[]> {
            ...
            this.executeCommandWithUtf8Output(`powershell -Command "${command}"`)
                .then((resultString) => {
                    ...
                    const allCurrentApps = alreadyKnownApps
                        ...
                        .concat(result.NewApps.map(this.convertNewAppToUwpApplication));
    ...
    private static readonly getUwpAppsCommand = `
    ...
    private static convertNewAppToUwpApplication(newApp: { AppId: string; DisplayName: string; LogoBase64: string; }): UwpApplication {
        return {
            appId: newApp.AppId,
            icon: {
